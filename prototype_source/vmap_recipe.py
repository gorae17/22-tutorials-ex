...
# 하는 것입니다. 예제와 같이 실행되는 함수 `func` 을 작성한 후 `vmap(func)` 을 통해 
# 예제의 배치들(batches)을 다루는 함수로 리프팅(lift) 할 수 있습니다. 그러나 
# `func` 에는 많은 제한이 있습니다.
#
# - 기존의 PyTorch 기능을 제외하고, 반드시 동작해야 합니다. (Python 
#   데이터 구조를 변경할 수 없습니다.)
# - 배치는 반드시 Tensor로 제공되어야 합니다. 즉, vmap은 특별한 설정 없이는 
#   가변 길이 시퀀스를 다룰 수 없습니다.
#
